...
def get_outputs(inputs, skip=False):
    ...
    with tf.variable_scope("Model") as scope:
        ...
        current_autoenc = autoenc_upsample #learning attention
        ...
        mask = tf.cast(tf.greater_equal(mask_a, transition_rate), tf.float32)
        mask_a_on_a = tf.multiply(images_a, mask)
        mask = tf.cast(tf.greater_equal(mask_b, transition_rate), tf.float32)
        mask_b_on_b = tf.multiply(images_b, mask)
...
def autoenc_upsample(inputae, name):
    ...
    with tf.variable_scope(name):
        ...
        pad_input = tf.pad(inputae, [[0, 0], [ks, ks], [
            ks, ks], [0, 0]], padding)
        o_c1 = layers.general_conv2d( pad_input, tf.constant(True, dtype=bool), ngf, f, f, 2, 2, 0.02, name="c1")
