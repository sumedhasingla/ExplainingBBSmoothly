...
import pandas as pd
import sys
sys.path.append("/pghbio/dbmi/batmanlab/singla/Explanation_XRay/")
from cycleGAN import cyclegan_datasets
from cycleGAN import data_loader, losses, model

tf.set_random_seed(1)
np.random.seed(0)
slim = tf.contrib.slim

main_dir = '/pghbio/dbmi/batmanlab/singla/Explanation_XRay'
...
class CycleGAN:
    ...
    def __init__(self, pool_size, lambda_a,
        ...
        self.fake_images_A = [] #pool
        self.fake_images_B = [] #pool
    ...
    def compute_losses(self):
        ...
        *_summ -> Summary variables for above loss functions
        ...
        d_loss_A = self._lambda_a *  losses.lsgan_loss_discriminator(
            prob_real_is_real=self.prob_real_a_is_real,
            prob_fake_is_real=self.prob_fake_pool_a_is_real,
        )
        d_loss_B = self._lambda_b *  losses.lsgan_loss_discriminator(
    ...
    def save_images(self, sess, epoch, curr_tr):
        ...
        names = ['inputA_', 'inputB_', 'mask_a', 'mask_b',
                 'mask_a_on_a_', 'mask_b_on_b_',
                 'fake_images_b_from_g_', 'fakeB_',
                 'fake_images_a_from_g_', 'fakeA_',
                 'mask_acycle_', 'mask_acycle_on_fakeA_',
                 'mask_bcycle_','mask_bcycle_on_fakeB_',
                 'cycle_images_a_from_g_',  'cycle_images_b_from_g_',
                 'cycA_', 'cycB_']
        ...
        with open(os.path.join(
            ...
            for i in range(0, self._num_imgs_to_save):
                ...
                fake_A_temp, fake_B_temp, cyc_A_temp, cyc_B_temp, masks, masked_imgs, masked_gen_imgs = sess.run([
                    self.fake_images_a,
                    self.fake_images_b,
                    self.cycle_images_a,
                    self.cycle_images_b,
                ...
                tensors = [inputs['images_i'], inputs['images_j'],  masks[0], masks[1], 
                           masked_imgs[0], masked_imgs[1],
                           masked_gen_imgs[0], fake_B_temp,
                           masked_gen_imgs[1], fake_A_temp,
                           masks[2], masked_imgs[2],
                           masks[3], masked_imgs[3],
                           masked_gen_imgs[2], masked_gen_imgs[3],
                           cyc_A_temp, cyc_B_temp,]
    ...
    def save_images_bis(self, sess, epoch):
        ...
        with open(os.path.join(self._output_dir, 'results_' + str(epoch) + '.html'), 'w') as v_html:
            ...
            for i in range(0, self._num_imgs_to_save):
                print("Saving image {}/{}".format(i, self._num_imgs_to_save))
                inputs = sess.run(self.inputs)
                #inputs['images_i'] = np.reshape(inputs['image_i'] , [1,model.IMG_HEIGHT, model.IMG_WIDTH, model.IMG_CHANNELS])
                #inputs['images_j'] = np.reshape(inputs['image_j'] , [1,model.IMG_HEIGHT, model.IMG_WIDTH,model.IMG_CHANNELS])
                ...
                for name, tensor in zip(names, tensors):
                    ...
                    if 'mask_' in name:
                        ...
                        image_name = name + str(i) + ".npy"
                        np.save(os.path.join(self._images_dir, image_name),
                               (np.squeeze(tensor[0])[:,:,0])
                               )
                        image_name = name + str(i) + ".jpg"
                    else:
                        imsave(os.path.join(self._images_dir, image_name),
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(sess.run(self.global_step), self._max_step):
                ...
                for i in range(0, max_images):
                    ...
                    _,summary_str, b_real, b_fake = sess.run(
                        [self.d_B_trainer, self.d_B_loss_summ, self.prob_real_b_is_real, self.prob_fake_pool_b_is_real],
                        feed_dict={
                            self.input_a:
                                inputs['images_i'],
                            self.input_b:
    ...
    def test(self):
        ...
        with tf.Session() as sess:
            sess.run(init)
            print("Checkpint dir: " , self._checkpoint_dir)
            chkpt_fname = tf.train.latest_checkpoint(self._checkpoint_dir)
            saver.restore(sess, chkpt_fname)
...
def parse_args():
    ...
    parser.add_argument('--config_filename', type=str, default='configs/MIMIC_CycleGAN_256_Cardiomegaly.yaml', help='The name of the configuration file.')
...
def main():
    ...
    if args is None:
        exit()
    config_path = os.path.join(main_dir,args.config_filename)
    config = yaml.load(open(config_path))
    print(config)
        
    name = config['name']
    log_dir = os.path.join(main_dir, config['log_dir'], config['name'])
    checkpoint_dir = os.path.join(main_dir, config['checkpoint_dir'])
    to_train = config['to_train']
    skip = config['skip']
    switch = config['switch']
    threshold_fg = config['threshold']
