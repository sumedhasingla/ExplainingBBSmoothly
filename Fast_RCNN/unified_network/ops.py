...
def rpn2proposal(cls, reg, anchors, NUMS_PROPOSAL, NMS_THRESHOLD, IMG_H, IMG_W):
...
def rpn2proposal_batch(cls1, reg1, anchors, NUMS_PROPOSAL, NMS_THRESHOLD, IMG_H, IMG_W):
    anchors = tf.constant(anchors, dtype=tf.float32)
    first_dim = cls1.get_shape().as_list()[0]
    normal_bbox_list = []
    reverse_bbox_list = []
    bbox_idx_list = []
    for i in range(first_dim):
        cls, reg = cls1[i], reg1[i]        
        normal_bbox, reverse_bbox = offset2bbox(reg, anchors)
        score = tf.nn.softmax(cls)[:, 1]
        box_idx = tf.image.non_max_suppression(reverse_bbox, score, max_output_size=NUMS_PROPOSAL, iou_threshold=NMS_THRESHOLD)
        reverse_bbox = tf.nn.embedding_lookup(reverse_bbox, box_idx)
        ...
        normal_bbox_list.append(normal_bbox)
        reverse_bbox_list.append(reverse_bbox)
        bbox_idx_list.append(bbox_idx)
        
        
    normal_bbox = tf.concat(normal_bbox_list, axis=0)
    reverse_bbox = tf.concat(reverse_bbox_list, axis=0)
    bbox_idx = tf.concat(bbox_idx_list, axis=0)
    
    return normal_bbox, reverse_bbox, bbox_idx

def non_max_suppression(cls, boxes, CLASSES, nms_threshold=0.1):
    box_list = []
    score_list = []
    class_list = []
