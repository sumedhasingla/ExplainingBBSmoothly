  ...
  with variable_scope.variable_scope(scope, 'vgg_a', [inputs]) as sc:
    ...
    with arg_scope(
      ...
      net = layers_lib.repeat(
          inputs, 1, layers.conv2d, 64, [3, 3], scope='conv1', trainable=False)
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool1')
      net = layers_lib.repeat(net, 1, layers.conv2d, 128, [3, 3], scope='conv2', trainable=False)
...
def vgg_16(inputs,
  ...
  with variable_scope.variable_scope(scope, 'vgg_16', [inputs]) as sc:
    ...
    with arg_scope(
      ...
      #print("net: ",net) 
      net = layers_lib.max_pool2d(net, [2, 2], scope='pool5')#(?, 8, 8, 512)
      print("net: ",net) 
      # Use conv2d instead of fully_connected layers.
      net = layers.conv2d(net, 4096, [8, 8], padding='VALID', scope='fc6')
      net = layers_lib.dropout(
           net, dropout_keep_prob, is_training=is_training, scope='dropout6')
      net = layers.conv2d(net, 4096, [1, 1], scope='fc7')
      # net = layers_lib.dropout(
      ...
      # Convert end_points_collection into a end_point dict.
      end_points = utils.convert_collection_to_dict(end_points_collection)
      return end_points["vgg_16/fc7"] #end_points["vgg_16/conv5/conv5_3"] #(?, 16, 16, 512),

def vgg_16_train(inputs,
           num_classes=1000,
           is_training=True,
           dropout_keep_prob=0.5,
           reuse=None,
  ...
  with variable_scope.variable_scope(scope, 'vgg_16', [inputs], reuse=reuse) as sc:
    ...
    with arg_scope(
      ...
      print("net: ",net)   
      prediction = tf.nn.sigmoid(net)
      return(net, prediction, end_points)
...
def vgg_19(inputs,
  ...
  Returns:
    the last op containing the log predictions and end_points dict.
  """
  with variable_scope.variable_scope(scope, 'vgg_19', [inputs],reuse=tf.AUTO_REUSE) as sc:
    ...
    with arg_scope(
      ...
      net = layers.conv2d(net, 4096, [shape[1], shape[2]], padding='VALID', scope='fc6')
      net = layers_lib.dropout(
          net, dropout_keep_prob, is_training=is_training, scope='dropout6')
      net = layers.conv2d(net, 4096, [1, 1], scope='fc7')
      net = layers_lib.dropout(
